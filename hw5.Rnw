...
\item Since $\mub = \X\b = \W\alphav$ does not depend on the model parameterization, we may wish that the posterior distribution of $\X\b$ equals the posterior distribution of $\W\alphav$ for all linear reparameterizations of the model of the form above. Restricting attention to  normal prior distributions with mean zero and precision $\phi \Phib_0$, show that this posterior distribution of $\X\b$ is the same as $\W\alphav$  if and only if $\Phib_0 = \C^T\C$ where $\mub \in C(\C)$.
