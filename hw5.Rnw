...
\begin{enumerate}
\item Let $ \Y \mid \b \sim \N_n(\X\b,\V/\phi)$. Find the conditional distribution of $\b$ given $\Y$, $\V >0$ and $\phi$ under the prior distribution $\b \sim \N(\bv_0,\Phib)$. In the case that $\bv_0 =\zero$, compare the posterior expectation of $\b$ to the GLS estimator.  (Try to express the posterior expectation in terms of the GLS estimator.)
...
\begin{enumerate}
\item Compute the bias, covariance  and MSE under squared error loss of this estimator as a function of $g$ and $\b$. 
\item Plot the MSE as a function of $g$, for fixed $\b$.  Overlay lines that show the squared bias term and the trace of the covariance matrix, along with a line for the MSE of the OLS estimator.  
...
\item Plot the MSE as a function of $\b$, for fixed $g$. for what values of $\b$ is the MSE of $\bhat_g$ lower than that of $\bhat$?  (Show in general)
...
\item Since $\mub = \X\b = \W\alphav$ does not depend on the model parameterization, we may wish that the posterior distribution of $\X\b$ equals the posterior distribution of $\W\alphav$ for all linear reparameterizations of the model of the form above. Restricting attention to prior distributions to mormal distributions with mean zero and precision $\phi \Phib_0$, show that this is possible if and only if $\Phib_0 = \C^T\C$ where $\mub \in C(\C)$.
...
\item With the estimate $\hat{\omega} \equiv (p-2) \sigma^2/\|\bhat\|^2$ as an Empirical Bayes estimate of the  $\omega$, solve for the estimate of $\sigma^2_0$ from 
